...
def Initialisation2D(imgps,t,ax,ay,params):
    ...
    # load camera images
    cv2.imread(params.casename+"/input/raw_images/c{cam}/c{cam}_{time}.tif".format(cam=str(0),time=str(time[0]).zfill(params.Zeros)))
    cam_imgs = [cv2.imread(params.casename+"/input/raw_images/c{cam}/c{cam}_{time}.tif".format(cam=str(params.cams[c]),time=str(time[0]).zfill(params.Zeros)),cv2.IMREAD_UNCHANGED) for cam in params.cams]
    ...
    print('found '+str(len(allTracks3D))+' 3D tracks')
